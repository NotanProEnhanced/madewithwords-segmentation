...
# restore-verify.sh lives beside this script. It used to be referenced as
# /root/restore-verify.sh, which only worked from a hand-placed copy; now that
# these live in the repo, resolve it relative to this file and fall back to the
# old location so an existing cron line keeps working.
HERE="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
VERIFY="$HERE/restore-verify.sh"
[ -x "$VERIFY" ] || VERIFY=/root/restore-verify.sh

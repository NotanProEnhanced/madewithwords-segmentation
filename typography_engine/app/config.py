...
# Word-variety dial for the displacement (Lifelike/Letter) renderer. DEFAULTS TO 0.0,
# which is the original engine byte-for-byte -- an environment that sets nothing is
# unchanged. Set TYPO_WORD_VARIETY=0.7 per-environment in .env (e.g. staging only) to
# flatten the 3x lead-word skew, so a varied word list reads as varied instead of being
# dominated by the first few words. Revert by deleting the line -- no code change.
...
# `keepsake` is a DEDICATED Loved in Words skin for redemptions — a separate slot from the
# public lovedinwords.com site AND the everloved pitch demo, so the whole redemption flow
# is one Loved in Words identity without touching either. Override via TYPO_REDEEM_BRAND.
REDEEM_BRAND = os.environ.get("TYPO_REDEEM_BRAND", "keepsake").strip() or "keepsake"

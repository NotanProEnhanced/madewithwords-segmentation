...
async def render(
    ...
    #               not name-dominated -- the "cloud" reading, distinct from Lifelike
    #   Passage  -> the message streamed in written order (flow, set above)
    disp_variety_eff = 1.0 if (disp_route and style_choice == "words") else WORD_VARIETY
    ...
    try:
        ...
        if pet_on:
            ...
            # The backdrop chips (Studio, Sand, ...) are what the human brands show; a chosen
            # one is the ground, as an explicit colour from Displacement's own table. Cutout
            # and the florals have no colour, so they fall back to the ink ground.
            from .pipeline.displacement import BACKDROPS as _BD
            _woven_rgb = None
